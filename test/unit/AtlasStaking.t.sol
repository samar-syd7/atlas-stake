...
contract AtlasStakingTest is Test {
    ...
    function setUp() public {
        ...
        staking = new AtlasStaking(address(stakingToken), address(rewardToken), INITIAL_REWARD_RATE);
    ...
    function testMultipleUsersRewardDistribution() public {
        ...
        assertEq(stakingToken.balanceOf(bob), 1000 ether);

        assertEq(stakingToken.allowance(bob, address(staking)), 100 ether);
        ...
        assertEq(aliceRewards, 15 ether);

        assertEq(bobRewards, 5 ether);
    }
    ...
    function testRewardDistributionAfterPartialWithdraw() public {
        ...
        console.log("Alice claimed", aliceBalance);
    ...
    function testFuzzRewardAccrual(uint96 amount, uint32 timePassed) public {
        ...
        vm.assume(timePassed > 0 && timePassed <= 365 days);
        ...
        uint256 expected = uint256(timePassed) * INITIAL_REWARD_RATE;

        uint256 maxDelta = uint256(amount) / 1 ether + 1;

        assertApproxEqAbs(rewards, expected, maxDelta);
    }
