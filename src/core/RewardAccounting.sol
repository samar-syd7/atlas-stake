...
abstract contract RewardAccounting is Storage {
    function _updatePool() internal {
        ...
        accRewardPerShare += (rewards * PRECISION_FACTOR) / totalStaked;
    ...
    function _pendingRewards(address account) internal view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimestamp && totalStaked > 0) {
            ...
            uint256 rewards = elapsed * rewardRatePerSecond;

            currentAccRewardPerShare += (rewards * PRECISION_FACTOR) / totalStaked;
        }

        return ((uint256(user.amount) * currentAccRewardPerShare) / PRECISION_FACTOR) - uint256(user.rewardDebt);
    }

    function _harvest(address account) internal returns (uint256 rewards) {
        _updatePool();

        rewards = _pendingRewards(account);
        ...
        require(newRewardDebt <= type(uint128).max, "Reward debt overflow");
